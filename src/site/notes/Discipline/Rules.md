---
{"dg-publish":true,"permalink":"/discipline/rules/","created":"2026-01-09T11:29:59.057+05:00","updated":"2026-01-15T12:58:04.577+05:00"}
---

##### Status
- I make Irada to follow these obligations for this month and at last resort pay charity defined below
##### How Rules Work
- Charity Fund = Al Burhan (PK97MEZN0003040103483355)
- Accountability Mentor (AM) = Mustafa Bilal Bhai
- Day starts at 0000
- Sudden Vacation can be taken in the event of an unforeseen circumstance, and all vacations to be taken with AM Approval
- 1 P = 50s proper Plank without cheating or Rs 1500
- If Ps are not completed before day end without valid unforeseen circumstance, the cash option becomes mandatory; uncompleted Ps carry forward 
- The Only Vacation Rules:
	- Salah Rules
	- Modesty Rules
	- Day Streaks 
	- Mamoolaat Rules
- Customized vacations can also be taken
##### Day Streaks
- Rs 20000 for breaking, for each 15 mins
- First look can't extend 2 seconds
- Hand sin counts as this 
##### Salah
- 5 P for missing Jammah Salah
- Missing masjid Jamah not allowed
- 5 P for not offering Witr or remaining Salahs before 0000
##### Health
- 5 P for breaking per day
- Consume only healthy foods, mentioned in [[Discipline/Final Diet Plan\|Final Diet Plan]]
- Eating out for no valid reason is not allowed
- There are 3 1-hour eating/drinking windows in 24 hours. Outside these only things allowed are mentioned in the plan.
- Other UCs: Someone asking to consume thrice without you asking
##### Prohibitions 
- 5 P a day of being immodest during opposite gender interaction
- Looking at media while feeling attraction counts as this
- Prohibitions of Mamoolat count as this
##### Pomos
- ~~Islamic Course/Islamic Book/Planning/Islamic Projects/Freelancing/Job Work/Career Learning/Skills~~
- ~~At least 4 Pomos based on Freelancing/Job Work/Job Search/Job Learning, or 3 Ps caused per Pomo~~ 
- Job Work/Tafseer Class allowed
- No Pomos on Sat Sun
- 3 P for Mandatory Pomos
- [[Keep/Deep Pomo Checklist\|Deep Pomo Checklist]]
- A Focus Session can be started anytime we like and for a set time interval and during that we must focus on the work at hand according to the focus requirements of Pomo rules, or a standard Pomo penalty is caused.
- A Pomo is only considered when a third unbiased person would consider it one, or an employer in the case of job hours.
- Mandatory Pomos
	- 1st before 1000
	- 2nd before 1100
	- 3rd before 1500
	- 4th before 1600
- Optional Pomos
	- 1st before 1600
	- 2nd before 1700
	- 3rd before 1700
##### Mamolaat
- 3 P per missing a protocol set
- Protocol set time ends when next prayer time enters, and it is 0000 for Isha protocol
- Vacation each Friday, but only 1 meal can be Level 3 and bellow. [[Discipline/Cheat Meals Tier List\|Cheat Meals Tier List]]
- Mamolaat Set List:
	- Tahajjud Protocol: Tahajjud ~~+ 1m Dua~~ 
	- Fajr Protocol: Fajr + Yaseen ~~+ Muraakba~~
	- Mandatory Pomos (Only check if done within time, no set penalty)
	- Chasht Protocol: ~~Ishraaq + Chasht +~~ Morning Azkaar (100 of each) + ~~Zikr Ism e Zaat~~
	- Optional Pomos (Only check if done within time, no set penalty)
	- Zhuhr Protocol: Zuhr + .25 Para ~~+ Zikr bil Jahr + [[Productivtiy/Our Task Management and Planning System/Daily Planning\|Daily Planning]]~~
	- Asr Protocol: Asr + Evening Azkaar (300 of Each) ~~+ Dua~~
	- Maghrib Protocol: Maghrib ~~+ Half Azkaar + Awabayn~~ + Surah Waqiah
	- Isha Protocol: Isha + Mulk + Witr
##### Digital Entertainment
- 3 P per 15 minutes of Ent or extra phone usage 
- 60 Minutes Phone Limit
	- Asr to Maghrib Phone allowed 
	- [[Tech/Digital Dumbing Guide\|Digital Dumbing Guide]]
	- Phone remains online 
- These don't count as Digital Entertainment: Nasheeds, Jackbox Party Games
- Opening an Ent feed counts as this
- Special UCs: Someone asking to consume, without you asking, 60 minutes phone online limit is lenient 
##### Sleep
- 5 P
- 2200-0300 Apr-Sep
- 2000-0400 Oct-Mar
- Not Sleeping between sunrise time and 9 AM

##### Classes: (Paused)
- Attending Tafseer class and Majlis Zikr, while reaching before the predefined time, is necessary
- 3 P
##### Unforeseen Circumstances 
- Unforeseen circumstances (UCs) can forgive penalties
- A UC validity can be checked when in doubt: it's valid if Qwen-3-Max calls it valid using this prompt, providing enough context, on the first attempt, without any clever cheating strategy: [[Discipline/Rules Accountability Prompt\|Rules Accountability Prompt]]